...
from utils.utils import get_n_params, get_node_data_loader, get_optimizer_and_lr_scheduler, get_ogb_evaluator
...
def evaluate(model: nn.Module, loader: dgl.dataloading.NodeDataLoader, loss_func: nn.Module,
    ...
    with torch.no_grad():
        ...
        for batch, (input_nodes, output_nodes, blocks) in enumerate(loader_tqdm):
            ...
            nodes_representation = model[0](blocks, input_features)
...
if __name__ == '__main__':
    ...
    hgconv = HGConv(graph=graph, input_dim_dict={ntype: graph.nodes[ntype].data['feat'].shape[1] for ntype in graph.ntypes}, hidden_dim=args['hidden_units'],
    ...
    for epoch in range(args['epochs']):
        ...
        train_accuracy = get_ogb_evaluator(predicts=train_y_predicts.argmax(dim=1), labels=train_y_trues)
        ...
        val_accuracy = get_ogb_evaluator(predicts=val_y_predicts.argmax(dim=1),
                                                                                labels=val_y_trues)
    ...
    train_accuracy = get_ogb_evaluator(predicts=train_y_predicts.argmax(dim=1),
    ...
    print(f'final valid accuracy {val_accuracy:.4f}')
    ...
    # save model result
    result_json = {
        "train accuracy": float(f"{train_accuracy:.4f}"),
        "validate accuracy": float(f"{val_accuracy:.4f}"),
        "test accuracy": float(f"{test_accuracy:.4f}")
    }
